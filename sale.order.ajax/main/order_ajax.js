...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		/**
		 * Initialization of sale.order.ajax component js_old
		 */
		init: function(parameters)
		{
			...
			if (this.params.USER_CONSENT === 'Y')
			{
                ...
                ajaxSubmit: function (obForm, callback)
                {
                    if (!obForm.target)
                    {
		...
		sendRequest: function(action, actionData)
		{
			...
			if (action === 'saveOrderAjax')
			{
				...
				//this.ajaxSubmit(BX('bx-soa-order-form'), BX.proxy(this.saveOrder, this));
			}
			else
			{
				BX.ajax({
		...
		getBlockFooter: function(node)
		{
			var sections = this.orderBlockNode.querySelectorAll('.bx-soa-section.bx-active'),
				...
				buttons = [];
                        ...
                        if(currentSection.id == this.basketBlockNode.id && currentSection && currentSection.id.indexOf(lastSection.id) != '-1')
                        {
				isLastNode = true;
                        }
		},
		...
		editSection: function(section)
		{
			...
			var active = false,
				...
				editButton, errorContainer;
                        ...
                        if (section.id == this.basketBlockNode.id && section.id == this.activeSectionId)
                            active = true;
                        else if (section.id != this.basketBlockNode.id)
                            active = true;
                        ...
                        if (section.id == this.basketBlockNode.id)
                        {
                            ...
                            if (this.result.SHOW_AUTH)
                            {
                                    BX.bind(editButton, 'click', BX.delegate(function(){
                                            this.animateScrollTo(this.authBlockNode);
		...
		editAuthorizeForm: function(authContent)
		{
			...
			login = this.createAuthFormInputContainer(
				window.arOptions['use_phone_as_login'] == 'Y' ? BX.message('STOF_PHONE') : BX.message('STOF_LOGIN'),
				BX.create('INPUT', {
					...
					events: {keypress: BX.proxy(this.checkKeyPress, this)}
                        ...
                        if (window.arOptions['use_phone_as_login'] == 'Y' && window.arOptions['phone_mask'] != "")
                            $(login).find('input').mask(window.arOptions['phone_mask']);
			...
			if (this.result.AUTH.new_user_registration_email_confirmation != 'Y')
			{
				...
				authFormNodes.push(
					BX.create('LABEL', {
                        ...
                        var obLoginIpnut = this.createAuthFormInputContainer(
							window.arOptions['use_phone_as_login'] == 'Y' ? BX.message('STOF_PHONE') : BX.message('STOF_LOGIN'),
							BX.create('INPUT', {
								props: {
									name: 'NEW_LOGIN',
                        ...
                        if (window.arOptions['use_phone_as_login'] == 'Y' && window.arOptions['phone_mask'] != "")
                            $(obLoginIpnut).find('input').mask(window.arOptions['phone_mask']);
		...
		getAuthReference: function(authContent)
		{
                    ...
                    if (BX(authContent).querySelector('.bx-soa-reg-block') !== null)
                        authContent = BX(authContent).querySelector('.bx-soa-reg-block');
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				title, label, itemNode, description;
			...
			label = BX.create('DIV', {
				...
				children: [
					...
					BX.create('DIV', {props: {className: 'bx-soa-pp-company-smalltitle'}, text: item.NAME})
				]
			});
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-line'},
		...
		selectPaySystem: function(event)
		{
			...
			var innerPsAction = BX.hasClass(target, 'bx-soa-pp-inner-ps') ? target : BX.findParent(target, {className: 'bx-soa-pp-inner-ps'}),
				actionSection = BX.hasClass(target, 'bx-soa-pp-company-graf-container') ? target : BX.findParent(target, {className: 'bx-soa-pp-company-graf-container'}),
			...
			else if (actionSection)
			{
				...
				{
					selectedSection = this.paySystemBlockNode.querySelector('.bx-soa-pp-company-line.bx-selected');
					BX.addClass(actionSection, 'bx-selected');
					actionInput = actionSection.querySelector('input[type=checkbox]');
					actionInput.checked = true;
			...
			for (i in delivery.EXTRA_SERVICES)
			{
				...
				else
				{
					serviceNode = BX.create('DIV', {
						...
						children: [
							BX.create('LABEL', {
								html: currentService.editControl + '<span>' + BX.util.htmlspecialchars(currentService.name) + '</span>'
								+ (currentService.price ? ' (' + currentService.priceFormatted + ')' : '')
								+ (currentService.description && currentService.description.length
									? '<div class="bx-soa-service-small">' + BX.util.htmlspecialchars(currentService.description) + '</div>'
									: '')
		...
		createDeliveryItem: function(item)
		{
			...
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, description, itemNode, logoNode, priceNode;
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company-line'},
				children: [label, priceNode, description],
		...
		selectDelivery: function(event)
		{
			...
			var target = event.target || event.srcElement,
				actionSection =  BX.hasClass(target, 'bx-soa-pp-company-line') ? target : BX.findParent(target, {className: 'bx-soa-pp-company-line'}),
				selectedSection = this.deliveryBlockNode.querySelector('.bx-soa-pp-company-line.bx-selected'),
				...
				if (settings.IS_PHONE == 'Y'){
					textNode.setAttribute('autocomplete', 'tel');
                                        if (window.arOptions['phone_mask'] != "")
                                            $(textNode).mask(window.arOptions['phone_mask']);
                                    }
