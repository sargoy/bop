...
function dadataUpdate() {
	const dadataToken = '5150c39a7f5eff6febe9c7593fa8329a78844f4d';
	...
	// mask
	$('#soa-property-3').mask('+7 (000) 000-0000');
...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		sendRequest: function(action, actionData)
		{
			...
			else if	(action === 'enterCoupon')
			{
				let data = this.getData(action, actionData)
				let that = this;

				$.ajax({
					url: '/local/ajax/coupon_bloger.php',
					method: 'POST',
					data: {
						coupon: data.coupon
					},
							...
							BX.ajax({
								...
								onsuccess: BX.delegate(function(result) {
									...
									if (result && result.order)
									{
										...
										that.deliveryCachedInfo = [];
										that.refreshOrder(result);
									}
									else
									{
										that.removeCoupon(result);
									}

									BX.cleanNode(that.savedFilesBlockNode);
									that.endLoader();
								}, that),
					...
					error: function () {
						Swal.fire({
							title: 'Ошибка!',
							text: 'К сожалению с добавлением купона временные трудности.',
							icon: 'error',
							showConfirmButton: false,
							timer: 2500
						});
		...
		showError: function(node, msg, border)
		{
			...
			if(/Ошибка регистрации нового пользователя: Пользователь с таким email/.test(msg)) {
				console.log(msg)
				msg = msg.concat(' ', '<a class="btn btn-primary" href="/my/" target="_blank">Авторизоваться</a>');
			}
		...
		editCoupons: function(basketItemsNode)
		{
			...
			BX.cleanNode(document.querySelector('.bx-soa-coupon'));
			document.getElementById('bx-soa-total').prepend(
				BX.create('DIV', {
		...
		getProfilesControl: function(node)
		{
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					BX.cleanNode(BX('custom_bx_soa_properties_profiles'));
					var customBlock = BX('custom_bx_soa_properties_profiles'),
						customProfile = BX.create('DIV', {
							props: {className: 'form-group bx-soa-location-input-container'},
							children: [label, profileChangeInput, input]
						});
					BX.prepend(customProfile, customBlock);
				}
				else
				{
					for (i in this.result.USER_PROFILES)
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			if (currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length)
			{
				period = BX.create('LI', {
					children: [
						BX.create('DIV', {props: {className: 'bx-soa-pp-list-termin'}, html: this.params.MESS_PERIOD + ': ' + currentDelivery.PERIOD_TEXT}),
						BX.create('DIV', {props: {className: 'bx-soa-pp-list-description'}})
					]
				});
			}
		...
		createDeliveryItem: function(item)
		{
			...
			// if (!checked && deliveryId === 1 || checked && deliveryId === 2) {
			// 	itemNode = BX.create('DIV', {
			// 		props: {className: 'bx-soa-pp-company'},
			// 		children: [label, title],
			...
			// 	});
			// } else
			if (!checked && deliveryId !== 2 && deliveryId !== 18) {
				itemNode = BX.create('DIV', {
					props: {
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				pickUpContentCol = BX.create('DIV', {props: {className: 'row'}});
		...
		initMaps: function()
		{
			...
			if (this.maps)
			{
				...
				if (this.params.SHOW_PICKUP_MAP === 'Y' && BX('pickUpMap'))
				{
					...
					$('#delivery_custom_map_box').hide();
					BX.cleanNode('delivery_custom_map');



					if (currentDelivery.ID !== '2') {
						$('#bx-soa-pickup').hide();
						$('#delivery_custom_map_box').show();

						this.resizeMapContainers(true);
						$('.bx-soa-pickup-list.main').remove();
						this.maps.initDeliveryMap(currentDelivery, this);
						...
						if (activeStores && activeStores.length) {
							$('#ID_DELIVERY_ID_1').closest('.bx-soa-pp-company').show();
		...
		resizeMapContainers: function(largeWidth = false)
		{
			...
			if (resizeBy && (pickUpMapContainer || propertyMapContainer))
			{
				...
				if (this.params.SHOW_PICKUP_MAP === 'Y' && pickUpMapContainer)
				{
					...
					if (largeWidth) {
						pickUpMapContainer.style.minWidth = 100+'%';
					}
				}
		...
		editPickUpList: function(isNew)
		{
			...
			var pickUpList = BX.create('DIV', {props: {className: 'bx-soa-pickup-list main col-md-4'}}),
			...
			if (!!isNew)
			{
				container = this.pickUpHiddenBlockNode.querySelector('.bx_soa_pickup>.row');
				if (!container)
					container = this.pickUpBlockNode.querySelector('.bx_soa_pickup>.row');
		...
		getStoreInfoHtml: function(currentStore)
		{
			...
			if (currentStore.ADDRESS)
				html += '<span class="custom_map_adress"><span>'+BX.message('SOA_PICKUP_ADDRESS') + ':</span> ' + BX.util.htmlspecialchars(currentStore.ADDRESS) + '</span>';

			if (currentStore.PHONE)
				html += '<span class="custom_map_phone">'+BX.message('SOA_PICKUP_PHONE') + ': ' + '<a href="tel:' + BX.util.htmlspecialchars(currentStore.PHONE) + '">'+BX.util.htmlspecialchars(currentStore.PHONE)+'</a></span>';

			if (currentStore.SCHEDULE)
				html += '<span class="custom_map_schedule">'+BX.message('SOA_PICKUP_WORK') + ': ' + '<span class="custom_map_schedule">с понедельника по пятницу</span>' + BX.util.htmlspecialchars(currentStore.SCHEDULE)+'</span>';

			if (currentStore.DESCRIPTION)
				html += '<span class="custom_map_description">'+BX.message('SOA_PICKUP_DESC') + ': ' + BX.util.htmlspecialchars(currentStore.DESCRIPTION)+'</span>';
							...
							BX.create('A', {
								...
								events: {
									click: BX.delegate(function(event){
										...
										// let deliveryAddress = $(event.target).parent().parent().find('.bx-soa-pickup-l-item-adress').text();
										// $("#soa-property-7").val('').val(deliveryAddress);
		...
		editTotalBlock: function()
		{
			...
			var total = this.result.TOTAL,
				priceHtml, params = {},
				discText, valFormatted, i, payIDChoose,
				curDelivery, deliveryError, deliveryValue, cartpaysesc,
			...
			{
				discText = '<strong>' + this.params.MESS_ECONOMY + '</strong>';
				cartpaysesc = "<br><small>(с учетом оплаты картой на сайте)</small>";
				if (total.DISCOUNT_PERCENT_FORMATED && parseFloat(total.DISCOUNT_PERCENT_FORMATED) > 0)
					discText += total.DISCOUNT_PERCENT_FORMATED;
				discText += cartpaysesc;

				this.totalInfoBlockNode.appendChild(this.createTotalUnit(discText, '- <strong>' + total.DISCOUNT_PRICE_FORMATED + '</strong>', {cartpay: true}));
			}
			...
			if (window.location.hostname !== 'bop.ru') {
				payIDChoose = '16';
			} else {
				payIDChoose = '9';
			}

			if(this.getSelectedPaySystem().ID === payIDChoose) {
				$('a.bx-soa-button').text('Оплатить')
			} else {
				$('a.bx-soa-button').text('Оформить')
			}
		...
		setAnalyticsDataLayer: function(action, id)
		{
			...
			for (i in this.result.GRID.ROWS)
			{
				if (this.result.GRID.ROWS.hasOwnProperty(i))
				{
					...
					/*gmtProducts.push({
						'sku': item.data.ID,  //артикул товара (обязательный параметр)
						'name': item.data.NAME,  //название товара (обязательный параметр)
						'category': (item.data[this.params.BRAND_PROPERTY + '_VALUE'] || '').split(', ').join('/'),  //категория, тип или вид товара
						'price': item.data.PRICE,  //стоимость (обязательный параметр)
						'quantity': item.data.QUANTITY  //количество (обязательный параметр)
			...
			switch (action)
			{
				...
				case 'purchase':
					/*gmtInfo = {
						'transactionId': id,  //id заказа (обязательный параметр)
						'transactionAffiliation': 'bop',  //название магазина где была осуществлена продажа;
						'transactionTotal': this.result.TOTAL.ORDER_TOTAL_PRICE,  //общая сумма транзакции (обязательный параметр)
						'transactionTax': this.result.TOTAL.TAX_PRICE,  //сумма налога;
						'transactionShipping': this.result.TOTAL.DELIVERY_PRICE,  //стоимость доставки
						'transactionProducts': gmtProducts
					}*/
					info = {
						'event': 'purchase',
						'ecommerce': {
