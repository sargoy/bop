...
if ($basketUserId) {
    ...
    if ($arData = $rsData->Fetch()) {
        foreach ($arData['UF_ELEMENT'] as $id) {
            if ($id) {
                $favElements[$id] = $id;
            }
        }
...
foreach ($this->basketItems as $row)
{
	$rowData = array(
		...
		'BRAND' => isset($row[$this->arParams['BRAND_PROPERTY'].'_VALUE'])
			...
			: '',
        'FAV' => $favElements[$row['PRODUCT_ID']] ? 'Удалить из избранного' : 'В избранное',
        'ACT' => $favElements[$row['PRODUCT_ID']] ? 'active' : '',
	);
...
$delivSummOrder = $this->arParams['DELIV_SUMM'];
if ($result['allSum'] < (int)$delivSummOrder) {
    $delivSummMustache = $delivSummOrder - $result['allSum'];
}
if ($result['DISCOUNT_PRICE_ALL'] > 0)
{
	$discount_sum = number_format(($result['allSum'] + $result['DISCOUNT_PRICE_ALL']) * 5 / 100, 0, '.', ' ');
} else {
	$discount_sum = number_format($result['allSum'] * 5 / 100, 0, '.', ' ');
}
//$totalPrice = $result['allSum'] - $discount_sum;
...
$totalData = array(
	...
	'DELIV_SUMM_DELTA' => number_format($delivSummMustache, 0, '.', ' ') === 0 ? number_format($delivSummMustache, 0, '.', ' ') : $delivSummMustache ,
