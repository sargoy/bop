...
<!--		<div class="smart-filter-title">Подбор параметров</div>-->
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smart-filter-form">
			...
			<div class="row">
				<?foreach($arResult["ITEMS"] as $key=>$arItem)//prices
				{
					...
					if(isset($arItem["PRICE"])):
						...
						<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-12<?endif?> mb-2 smart-filter-parameters-box bx-active">
							...
							<div class="smart-filter-parameters-box-title mt-3" onclick="smartFilter.hideFilterProps(this)">
								<span class="smart-filter-parameters-box-title-text"><?=$arItem["NAME"]?></span>
                                <span class="caret smart-filter-angle smart-filter-angle-up" data-role="prop_angle"></span>
							</div>
							<div class="smart-filter-block mt-4" data-role="bx_filter_block">
								<div class="smart-filter-parameters-box-container">
									<div class="smart-filter-input-group-number">
										<div class="d-flex justify-content-between filter-range">
				...
				{
					if (empty($arItem["VALUES"]) || isset($arItem["PRICE"]) || ($arParams['SECTION_ID'] == 442 && ($key == 586 || $key == 636)))
						continue;
					...
					<div class="<?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL"):?>col-sm-6 col-md-4<?else:?>col-lg-12<?endif?> mb-2 smart-filter-parameters-box <?if ($arItem["DISPLAY_EXPANDED"]== "Y"):?>bx-active<?endif?>">
						...
						<div class="smart-filter-block mt-3 mb-2" data-role="bx_filter_block">
							<div class="smart-filter-parameters-box-container">
							<?
							...
							switch ($arItem["DISPLAY_TYPE"])
							{
								...
								<div class="smart-filter-input-group-checkbox-list">
                                    <?php $arItem["VALUES"]['a'] = $arItem["VALUES"]['Оливковое']; ?>
                                    <?php $arItem["VALUES"]['b'] = $arItem["VALUES"]['Подсолнечное']; ?>
                                    <?php unset($arItem["VALUES"]['Оливковое']); ?>
                                    <?php unset($arItem["VALUES"]['Подсолнечное']); ?>
                                    <?php ksort($arItem["VALUES"]) ?>
                                    <?php $ar_mult = [] ?>
                                    <?foreach($arItem["VALUES"] as $val => $ar):?>
                                        <?php if (!$ar['VALUE']) continue; ?>
                                        <?php $name = trim(preg_replace('/\s+/u', ' ', $ar['VALUE'])); ?>
                                        <?php if ($ar_mult[$name]) continue ?>
                                        <div class="form-group form-check mt-1 mb-2 custom-control custom-checkbox">
                                            <input
                                                type="checkbox"
                                                ...
                                                onclick="smartFilter.click(this)"
                                            />
                                            <label data-role="label_<?=$ar["CONTROL_ID"]?>" class="smart-filter-checkbox-text form-check-label custom-control-label" for="<? echo $ar["CONTROL_ID"] ?>">
                                                <?=$name;
                                                if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && $ar["ELEMENT_COUNT"]):
                                                    ?>&nbsp;(<span data-role="count_<?=$ar["CONTROL_ID"]?>"><? echo $ar["ELEMENT_COUNT"]; ?></span>)<?
                                                endif;?>
                                            </label>
                                        </div>
                                        <?php $ar_mult[$name] = true ?>
                                    <?endforeach;?>
								</div>
							<?
								//endregion
			...
			<div class="row">
				<div class="col smart-filter-button-box">
					<div class="smart-filter-block">
						<div class="smart-filter-parameters-box-container mt-3">
							<input
							...
							<div class="smart-filter-popup-result <?if ($arParams["FILTER_VIEW_MODE"] == "VERTICAL") echo $arParams["POPUP_POSITION"]?>" id="modef" <?if(!isset($arResult["ELEMENT_COUNT"])) echo 'style="display:none"';?> style="display: inline-block;">
								Выбрано: <span id="modef_num"><?=intval($arResult["ELEMENT_COUNT"])?></span>
