...
<?foreach ($arResult['SECTIONS_FORMAT'] as $arSection):
    ...
    <div class="collapse-catalog-sidebar mb-2">
        <button class="btn collapse-toggle btn-link collapsed<?if($arSection['ACTIVE_LINK']):?> active-text-link<?endif;?>"<?if($cnt):?> data-id="<?=$arSection['ID']?>"<?endif;?> data-href='<?= isset($_GET['q']) ? "{$_SERVER['REQUEST_URI']}&sec={$arSection['ID']}" : "?sec={$arSection['ID']}"?>'>
            <span class="caption">
                ...
                <div class="collapse-content">
                    <ul class="list-unstyled mb-0">
						<?foreach ($arSection['PARENT'] as $k=>$arSubSection):?>
                            <li class="mb-3">
                                <a href='<?= isset($_GET['q']) ? "{$_SERVER['REQUEST_URI']}&sec={$arSubSection['ID']}" : "?sec={$arSubSection['ID']}"?>' class="font-weight-medium text-reset"><?=$arSubSection['NAME']?></a>
                            </li>
						<?endforeach;?>
                    </ul>
                </div>
